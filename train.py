...
def add_scalars(writer,epoch,loss_val,acc_val,):
...
def main(config,dataset):
    ...
    writer = SummaryWriter(log_dir=os.path.join(config.output.dir,'runs/result'),flush_secs=30)
    ...
    if args.dataset == 'cifar10':
        num_classes = 10
        train_dataset = datasets.CIFAR10(root=cifar10_data_dir, train=True, download=True, transform=transformer)
        val_dataset = datasets.CIFAR10(root=cifar10_data_dir, train=False, download=True, transform=transformer)
    elif args.dataset == 'cifar100':
        num_classes = 100
        train_dataset = datasets.CIFAR100(root=cifar100_data_dir, train=False, download=True, transform=transformer)
        val_dataset = datasets.CIFAR100(root=cifar100_data_dir, train=False, download=True, transform=transformer)
    else:
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=config.train.batch_size, shuffle=False,
                                             num_workers=config.train.workers, pin_memory=True, sampler=None)
    val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=config.val.batch_size, shuffle=False,
                                             num_workers=config.val.workers, pin_memory=True, sampler=None)
    ...
    for epoch in range(config.train.start_epoch, config.train.epochs):
        ...
        train_one_epoch(config, model, criterion, train_loader, optimizer, epoch, lr_scheduler, logger,writer=writer)
        if epoch % config.output.epoch_print_freq == 0 or epoch >= (config.train.epochs - 10):
            acc1, acc5, loss = validate(config, val_loader, model, logger,num_classes)
            logger.info(f"Valdation set Accuracy of the network at epoch {epoch}: Top1: {acc1:.3f}%, Top5: {acc5:.3f}%, Loss: {loss:.5f}")
            ...
            if epoch % config.output.save_freq == 0 or is_best:
                save_checkpoint(config, epoch, model, max_accuracy, loss, optimizer, lr_scheduler, logger, is_best=is_best)
    save_checkpoint(config, epoch, model, max_accuracy, loss, optimizer, lr_scheduler, logger, is_best=is_best)
