...
def build_loader(config):
    ...
    train_dataset = PracticalDataset(dataset_dir=root_dir,
                                     txt_file=train_txt_file,
                                     transforms=train_transforms)
    val_dataset = PracticalDataset(dataset_dir=root_dir,
...
def build_hs_loader_cifar100(config):
    ...
    trainset = datasets.CIFAR100(root=root_dir,
                                 train=True,
                                 download=False,
                                 transform=transform_train)
    trainloader = torch.utils.data.DataLoader(
        trainset,
        batch_size=config.train.hs.batch_size,
        shuffle=True,
        num_workers=4)

    testset = datasets.CIFAR100(root=root_dir,
                                train=False,
                                download=False,
                                transform=transform_test)
    testloader = torch.utils.data.DataLoader(
        testset,
        batch_size=config.val.hs.batch_size,
        shuffle=False,
        num_workers=4)
...
if __name__ == "__main__":
    train_dataset = PracticalDataset(
        dataset_dir="/home/bjy/data/aaai2023_challenge",
        txt_file="train_90p.txt",
        transforms=train_transforms)
